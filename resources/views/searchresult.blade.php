...
@section('content')
@if(!empty($data[2]))
{{!$catName=App\category::where('categoryID',$data[2])->first()}}
@endif
<title>Search result for {{ucwords($data[1])}} @if(!empty($data[2])) in {{ucwords($catName->categoryName)}} category @endif</title>
        ...
        <div class="row">
          <div id="primary" class="content-area">
            <main id="main" class="site-main" role="main">
              <h1 class="page-title">Search results: “{{ucwords($data[1])}}”
                @if(!empty($data[2])) in “{{ucwords($catName->categoryName)}}” Category @endif</h1>
              ...
              <div id="grid-view" class="woocommerce active">
                ...
                <ul class="products columns-3 enable-hover">
                  ...
                  No products were found matching your search.</p>
                </ul>
                @else
                  ...
                  <li class="post-20 product type-product status-publish has-post-thumbnail product_cat-cameras-photography-de pa_brands-sony last instock sale featured shipping-taxable purchasable product-type-variable has-default-attributes has-children" style="height: 386px;">
                    <div class="product-inner">
                      <a href="{{Config::get('constant.options.sitelink')}}product/{{$url}}" class="woocommerce-LoopProduct-link woocommerce-loop-product__link">
                        ...
                        @endif
                        @if(strtotime('+7 day', strtotime($product->created_at)) > time())
